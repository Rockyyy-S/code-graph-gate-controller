...
test("可信 registry sequence=6 绑定批准证据、候选提交、实现摘要和新 producer", async () => {
  ...
  assert.equal(record.sequence, 6);
  assert.equal(record.sourceCommit, "42d65db46488bb6b6391d4331a03f61bfa14c4e7");
  assert.equal(
  ...
  assert.equal(
    ...
    "6cb7866d00a64b3f5b25bd94a67a8dee47cff8525dca44d9493f163456ad812e",
  );
  assert.equal(record.approvalEvidenceDigest, sha256CanonicalJson(approval));
  assert.equal(approval.sequence, record.sequence);
  assert.equal(approval.producerWorkflowSha, "7625ab3c58d6e0fd4d6b16e577609ab96b127870");
  assert.doesNotThrow(() =>
    validateTrustedRegistryApproval({
      approval,
