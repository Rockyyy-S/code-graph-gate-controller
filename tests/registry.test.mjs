...
test("可信 registry sequence=5 绑定批准证据、候选提交、实现摘要和新 producer", async () => {
  ...
  assert.equal(record.sourceCommit, "746e2fec4114f29aad9e0fb8b6b5dbf2ab2ca2c2");
  assert.equal(
  ...
  assert.equal(
    ...
    "b35ff644cdfa55518969af6f9afdb3158ccf9700757a114b6071199c345f240e",
  );
  assert.equal(record.approvalEvidenceDigest, sha256CanonicalJson(approval));
  assert.equal(approval.sequence, record.sequence);
  assert.equal(approval.producerWorkflowSha, "d69682a9b4342acaa7048eaf664a77e8d35e1da1");
  assert.doesNotThrow(() =>
    validateTrustedRegistryApproval({
      approval,
