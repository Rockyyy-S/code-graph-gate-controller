...
test("可信 registry sequence=7 绑定批准证据、候选提交、实现摘要和新 producer", async () => {
  ...
  assert.equal(record.sourceCommit, "f77d3d8b308b7b772492660204e72d210e46dd97");
  assert.equal(
  ...
  assert.equal(
    ...
    "10713ae32ddbd9efbcd375ebdbe73fb8c48bf51b47a36a0bf100e5dc06117b51",
  );
  assert.equal(record.approvalEvidenceDigest, sha256CanonicalJson(approval));
  assert.equal(approval.sequence, record.sequence);
  assert.equal(approval.producerWorkflowSha, "0792e37950ea90cd653563b571811e1e1628dfd8");
  assert.doesNotThrow(() =>
    validateTrustedRegistryApproval({
      approval,
