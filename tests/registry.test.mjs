...
test("可信 registry sequence=9 绑定批准证据、候选提交、实现摘要和新 producer", async () => {
  ...
  assert.equal(record.sequence, 9);
  assert.equal(record.sourceCommit, "eb7b99e16706ad322b4aa8a08675fc1ca5aad5a4");
  assert.equal(
  ...
  assert.equal(
    ...
    "8428a0e9cd83e889a053130e7cba84a2457cf7132db1f564c9d0ccb8f43a5a01",
  );
  assert.equal(record.approvalEvidenceDigest, sha256CanonicalJson(approval));
  assert.equal(approval.sequence, record.sequence);
  assert.equal(approval.producerWorkflowSha, "408d16a088d5b1476c6ac7fe1a8a5059779e6dc4");
  assert.doesNotThrow(() =>
    validateTrustedRegistryApproval({
      approval,
