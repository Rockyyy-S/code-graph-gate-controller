...
test("可信 registry sequence=8 绑定批准证据、候选提交、实现摘要和新 producer", async () => {
  ...
  assert.equal(record.sourceCommit, "bee9814b547ee802485081307bde757b47de6a98");
  assert.equal(
  ...
  assert.equal(
    ...
    "30a104e7a7776b130560245c6ee90e34e93bd9a9b6211223bcd7a35edc3b7c7a",
  );
  assert.equal(record.approvalEvidenceDigest, sha256CanonicalJson(approval));
  assert.equal(approval.sequence, record.sequence);
  assert.equal(approval.producerWorkflowSha, "4da26e84382c7185c37304dc73fb32fb2ad1d579");
  assert.doesNotThrow(() =>
    validateTrustedRegistryApproval({
      approval,
