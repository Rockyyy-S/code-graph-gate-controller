...
test("可信 registry sequence=10 绑定批准证据、候选提交、实现摘要和新 producer", async () => {
  ...
  assert.equal(record.sequence, 10);
  assert.equal(record.sourceCommit, "7bf20c9d8d2ded763c5252786d6060490c96ef0e");
  assert.equal(
  ...
  assert.equal(
    ...
    "55470a32ece8a4be5872d85ff7b8acddea974034049b8bc7aeff2e001c250c90",
  );
  assert.equal(record.approvalEvidenceDigest, sha256CanonicalJson(approval));
  assert.equal(approval.sequence, record.sequence);
  assert.equal(approval.producerWorkflowSha, "2327559590f0f377c56f9cae0d51544dea78bde5");
  assert.doesNotThrow(() =>
    validateTrustedRegistryApproval({
      approval,
