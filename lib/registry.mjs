...
export function validateProposedRegistryApproval({
  ...
  if (
    ...
    approval.sourceCommit !== record.sourceCommit ||
    approval.effectiveAt !== record.effectiveAt ||
...
export async function loadApprovedProposals(
  ...
  const recordNames = entries
    ...
    .sort();
  const proposals = [];
  const controllerMode = expectedProducerWorkflowSha === undefined;
  for (const recordName of recordNames) {
    ...
    validateProposedRegistryApproval({
      ...
      expectedProducerWorkflowSha: controllerMode
    ...
    const effectiveAt = Date.parse(record.effectiveAt);
    const expiresAt = Date.parse(record.expiresAt);
    // Controller 必须保留 exact-head 的失效 proposal 以阻止 canonical fallback；Harness 仍只消费有效窗口。
    if (controllerMode || (now >= effectiveAt && now <= expiresAt)) {
...
/**
 * 为 Controller 原子选择候选 record 与 producer 授权。
 *
 * exact-head proposal 在 canonical fallback 前判定；一旦存在就必须完整通过时间窗、摘要和 producer 闭合。
 */
export function selectCandidateAuthorization({
  ...
  validateTrustedRegistryRecord(currentRecord);
  if (!/^[a-f0-9]{40}$/u.test(canonicalProducerWorkflowSha)) {
    throw new Error("canonical producer workflow SHA 无效。\n");
  }
  ...
  if (matches.length > 1) {
    throw new Error("候选对同一精确 PR head 存在多个或冲突的 proposed 批准。\n");
  }
  if (matches.length === 0) {
    if (
  ...
  const [{ approval, record }] = matches;
  const producerWorkflowSha = expectedProducerWorkflowSha ?? approval.producerWorkflowSha;
  validateProposedRegistryApproval({
    approval,
    currentRecord,
  ...
  if (record.gateRegistryDigest !== registryDigest) {
    throw new Error("精确 PR head 的 proposed registry digest 与候选不一致。\n");
  }
  return {
    producerWorkflowSha,
    record: normalizeProposedRecord(record),
  };
}

/** Harness 保留 expected-workflow 校验，并只返回其既有可信 record 视图。 */
export function selectTrustedRecordForCandidate({
  ...
  return selectCandidateAuthorization({
...
/** 把 proposed 专属字段裁剪为 Controller/Harness 共用的 TrustedGateRegistryRecordV1。 */
function normalizeProposedRecord(record) {
  return {
