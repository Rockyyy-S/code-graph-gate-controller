...
import { isNoOpRootScript } from "./gate-command-policy.mjs";

/**
 * 影响 Gate 真实执行语义的受保护文件集合。
 *
 * 该列表由不可变 GateHarness 持有，候选提交不能自行缩小；普通产品测试不进入信任根，
 * 但公共能力 binding 显式引用的专属测试/fixture 会动态加入摘要。
 */
...
const PUBLIC_CAPABILITY_BINDINGS_PATH = "ci/public-capability-gates.v1.json";
...
export async function computeGateImplementationDigest(
  ...
  for (const { gateDefinition } of registry.gates) {
    ...
    if (executable === "pnpm" && args.length === 1) {
      ...
      if (isNoOpRootScript(implementation)) {
        throw new Error(`gate ${gateDefinition.gateId} 的根脚本是恒成功或内联 no-op。`);
      }
  ...
  const capabilityVerificationPaths = await collectCapabilityVerificationPaths(
    candidateRoot,
  );
...
/**
 * 读取候选能力 binding，把专属入口、测试和 fixture 加入不可变 Harness 的实现投影。
 */
async function collectCapabilityVerificationPaths(candidateRoot) {
  let source;
  try {
    source = await readFile(
      path.join(candidateRoot, ...PUBLIC_CAPABILITY_BINDINGS_PATH.split("/")),
      "utf8",
    );
  } catch (error) {
    if (error && typeof error === "object" && error.code === "ENOENT") {
  ...
  try {
    ...
    throw new Error(`${PUBLIC_CAPABILITY_BINDINGS_PATH} 不是合法 JSON。`);
  }
  if (value?.schemaVersion !== 1 || !Array.isArray(value.bindings)) {
    throw new Error(`${PUBLIC_CAPABILITY_BINDINGS_PATH} 缺少 schemaVersion=1 bindings。`);
  }
  const paths = new Set([PUBLIC_CAPABILITY_BINDINGS_PATH]);
  for (const binding of value.bindings) {
    if (binding?.verification === undefined) {
      continue;
    }
    const assertionTarget = binding.verification.assertionTarget;
    if (
      assertionTarget === null ||
      typeof assertionTarget !== "object" ||
      !isSafeRelativePath(assertionTarget.modulePath) ||
      !/^[A-Za-z_$][A-Za-z0-9_$]*$/u.test(assertionTarget.exportName ?? "")
    ) {
      throw new Error(
        `${PUBLIC_CAPABILITY_BINDINGS_PATH} 的 assertionTarget 未绑定安全模块导出。`,
      );
    }
    paths.add(assertionTarget.modulePath);
    for (const field of ["entryPath", "testPath", "fixturePath"]) {
      const relativePath = binding.verification[field];
      if (!isSafeRelativePath(relativePath)) {
        throw new Error(
          `${PUBLIC_CAPABILITY_BINDINGS_PATH} 的 ${field} 不是安全仓库相对路径。`,
        );
      }
