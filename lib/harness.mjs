...
export async function produceGateEvidence(options) {
  ...
  for (const entry of registry.gates) {
    ...
    if (remainingMs <= 0) {
      ...
      const runtime = await prepareGateRuntime(options, definition.gateId);
      try {
...
/** 为单个 gate 派生不可与其他 gate 共享的 HOME/TMP 路径。 */
export function createGateRuntimePaths(options, gateId) {
  return {
    gateHome: path.join(options.gateHome, gateId),
    gateTempDirectory: path.join(options.gateTempDirectory, gateId),
  };
}
...
async function prepareGateRuntime(options, gateId) {
  ...
  const runtime = createGateRuntimePaths(options, gateId);
  await Promise.all([
...
/** pkill 返回 1 仅表示该 UID 当前没有残留进程。 */
async function terminateGateIdentity(gateUid) {
  try {
    await execFileAsync("pkill", ["-KILL", "-u", `${gateUid}`]);
  } catch (error) {
    if (error && typeof error === "object" && error.code === 1) {
      return;
    }
    throw new Error("无法清理 gate 隔离 UID 的残留进程。", { cause: error });
  }
}
