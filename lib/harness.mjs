...
/**
 * 从 proposed 目录为 Harness 选择当前候选的可信记录。
 *
 * 目录加载阶段只校验每个 pair 自身的结构、canonical base 和摘要闭合，不能把当前 run
 * producer 提前施加到无关 head。必须先完成 exact-head 唯一选择，再仅对选中的 proposal
 * 强制当前 producer 与有效时间窗，避免旧的合法 proposal 阻断当前候选，同时保持 fail closed。
 */
export async function loadHarnessTrustedRecordForCandidate({
  currentRecord,
  headOid,
  now = Date.now(),
  proposedRecordDirectory,
  providerRepositoryId,
  pullNumber,
  registryDigest,
  workflowSha,
}) {
  const proposals = await loadApprovedProposals(proposedRecordDirectory, {
    currentRecord,
    now,
  });
  return selectTrustedRecordForCandidate({
    currentRecord,
    headOid,
    now,
    proposals,
...
export async function produceGateEvidence(options) {
  ...
  const trustedRecord = await loadHarnessTrustedRecordForCandidate({
    currentRecord: currentTrustedRecord,
    headOid: options.headOid,
    proposedRecordDirectory: options.proposedRecordDirectory,
